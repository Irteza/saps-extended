...
int Simulator::command(int argc, const char*const* argv) {
	...
	if (argc == 3) {
		...
		if (strcmp(argv[1], "initialize-incoming-traffic-matrix") == 0) {
			// incomingTrafficPerLeaf
			numLeafs_ = atoi(argv[2]);
			initializeIncomingTrafficPerLeaf();
			return TCL_OK;
		} // added by SMI 24-June-2017
	}
	if (argc == 4) {
		if (strcmp(argv[1], "add-node") == 0) {
			...
			add_node(node, id);
			return TCL_OK;
		} else if(strcmp(argv[1], "get-incoming-traffic") == 0) {
			int incomingTrafficToLeafCount = getIncomingTrafficToLeaf(atoi(argv[2]), atoi(argv[3])); // 
			tcl.resultf("%d", incomingTrafficToLeafCount); // get-incoming-traffic %d %d", srcLeaf_, destLeaf_
			return TCL_OK;
		} 
	}
	if (argc == 5) {		
		if (strcmp(argv[1], "update-incoming-traffic-matrix") == 0) {
			// parameters (src_leaf_ind dst_leaf_ind 1)
			updateIncomingTrafficMatrix(atoi(argv[2]), atoi(argv[3]), atoi(argv[4]));
			return TCL_OK;
		} // added by SMI 24-June-2017
	}
...
/* Initialize the incoming traffic matrix for each leaf switch - SMI 24-June-2017    */
void Simulator::initializeIncomingTrafficPerLeaf() {	
	incomingTrafficPerLeaf = new int*[numLeafs_];

	for (int i = 0; i < numLeafs_; ++i) {
		incomingTrafficPerLeaf[i] = new int[numLeafs_];
	}

	for (int i = 0; i < numLeafs_; ++i) {
		for (int j = 0; j < numLeafs_; ++j) {
			incomingTrafficPerLeaf[i][j] = 0;
		}
	}
}

/*  Update the statistics for a particular src leaf and dest leaf pair - SMI 24-June-2017 */
void Simulator::updateIncomingTrafficMatrix(int src_leaf, int dest_leaf, int change) {
	if((src_leaf < numLeafs_) && (dest_leaf < numLeafs_)) {
		incomingTrafficPerLeaf[src_leaf][dest_leaf] = incomingTrafficPerLeaf[src_leaf][dest_leaf] + change;
	}
}

/*  Return count for a specific src leaf and dest leaf pair - SMI 24-June-2017 */
int Simulator::getIncomingTrafficToLeaf(int src_leaf, int dest_leaf) {
	int count = 0;
	if((src_leaf < numLeafs_) && (dest_leaf < numLeafs_)) {
		for(int i=0; i < numLeafs_; i++) {
			if((i != src_leaf) && (i != dest_leaf)) {
				count = count + incomingTrafficPerLeaf[i][dest_leaf];
			}
		}
	}
	return count;
